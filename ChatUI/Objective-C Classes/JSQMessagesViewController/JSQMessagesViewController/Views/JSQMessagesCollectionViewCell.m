...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    UIButton *btnTemp;
    NSLog(@"Subviews are %@", self.messageBubbleContainerView.subviews);
    for (UIView *view in self.messageBubbleContainerView.subviews) {
        if ([view isKindOfClass:[UIButton class]]) {
            btnTemp = (UIButton *) view;
        }
    }
    
    for (UIImageView *view in self.messageBubbleContainerView.subviews) {
        if ([view isKindOfClass:[UIButton class]]) {
            btnTemp = (UIButton *) view;
        }
    }
    
    if ([touch.view isDescendantOfView:btnTemp]) {
        return NO;
    }
